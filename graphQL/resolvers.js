...
module.exports = {
  ...
  Query: {
    ...
    getAllLocation: async () => {
      try {
        const allLocations = await Location.find().populate("rmanager");
  ...
  Mutation: {
      ...
      try {
        ...
        if (existingCoach) {
          ...
          location.rmanager.map(async rmID => {
            ...
            await sendEmail("lakaganth89@gmail.com", "Laka");
            await rmManager.save();
          });
          return location;
        }
    ...
    addClassToCoach: async (root, { addClassInput }, ctxt) => {
      ...
      try {
        ...
        const coach = await Coach.findOne({ coach_name });
    ...
    deleteAll: async () => {
      ...
      report.map(async c => {
        await Report.findByIdAndRemove(c);
      });
